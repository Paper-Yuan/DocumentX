 ...
 * computer-design/desktop_hub/crypto_protocol.js so the two implementations stay wire-compatible.
...
class CryptoEngineTest {
    ...
    private fun loadVectors(): JsonObject {
        // JUnit's working directory is the Gradle module directory, measured as
        // <repo>/android-design/com/app, so the repository root is three levels up.
        val file = File("../../../test/vectors/e2e-v2.json")
        assertTrue(
            "cross-end vectors not found at ${file.absolutePath} (working dir ${File(".").absolutePath}); " +
                    "they are committed under test/vectors and must be present for this contract test",
            file.isFile
        )
        return Gson().fromJson(file.readText(Charsets.UTF_8), JsonObject::class.java)
    }
